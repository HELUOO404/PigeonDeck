...
- **复冒烟微调（`364b5c5`, `910ea28`）**：① 修工具盘收起时 **Logo 上下闪现**——收起动画期 fading 工具盘仍在流内，立即翻底锚会把球顶上去再弹回；改为过渡结束后再翻锚（位置重合无跳变）。② 修**智能↔高级样式切换卡顿**——切回较高的智能样式时主体 `.pbody` 过渡中被挤出滚动条、到终高又消失致回流；动画期 `.panel.pd-h-anim .pbody{overflow:hidden}` 隐藏滚动条，静止态自适应无条。
