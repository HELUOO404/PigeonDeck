...
真机使用反馈修复，按 6 组分批实施（每组独立提交、逐组五道门禁 + 关键路径 diff 通读 + E2E ×2 复核）。基线：build ✓ / typecheck ✓ / vitest 351 ✓ / 全量 E2E 96 passed ×2 ✓ / i18n ✓（唯 `copy-text②`、`full-flow` 因本机 headed 环境不授予剪贴板**读**权限失败，已 bisect 确认 pre-W1 同样失败，属已知环境限制，非回归）。
