...
## 5. 编辑面统一按键
- **所有文字编辑面：保存快捷键（默认 `Ctrl/Cmd+Enter`）= 保存/提交，`Esc` = 不保存退出。** 覆盖：批注说明 textarea、区域说明、内联富文本、替换媒体、复制文本可编辑预览。新加任何编辑面必须两键齐备：保存键统一读 `settings.shortcuts.save`（经 [[shortcuts]] `matchCombo`，定义在 `src/state/shortcuts-def.ts` 且可在 设置 → 快捷键 重绑，护栏强制含修饰键）；`Esc` 走各面自身处理。
