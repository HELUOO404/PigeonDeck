...
/**
 * 逻辑12：默认选择粒度已改为 'element'（移动选原始命中、批注不显示 +/- 胶囊）。
 * 以下用例验证「智能组件块」基准下的行为，需先进设置切到 smart 再回批注模式。
 * 共享 settings 引用 + 同一 SelectionResolver 实例 → 即时对移动/面板生效。
 */
async function setGranularitySmart(page: Page): Promise<void> {
  await clickShadowEl(page, 'pd-btn-settings');
  await waitShadowTestId(page, 'pd-set-gran-smart');
  await clickShadowEl(page, 'pd-set-gran-smart');
  await clickShadowEl(page, 'pd-settings-close');
  await page.waitForFunction(
    () => !document.getElementById('pd-host')?.shadowRoot?.querySelector('[data-testid="pd-settings"]')
  );
}
...
test('⑤  多级 +/- 粒度不过冲：连点 + 两次落在正确的 2 级祖先', async () => {
  ...
  // 逻辑12：默认粒度改为 element 后 +/- 胶囊默认隐藏；本用例测 smart 基准的 +/- 重指向
  await setGranularitySmart(page);
  ...
  // 保存：点顶部文本框聚焦（避开右下工具盘遮挡区）→ 填批注 → Ctrl+Enter 保存
  await clickShadowEl(page, 'pd-panel-note');
  await page.keyboard.type('粒度解析测试');
