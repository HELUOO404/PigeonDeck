 ...
 * ③ 语言快切（F11：2 项下拉 英文/跟随界面）：点 pd-output-lang → 选「跟随界面」
 *    → resolveLang('auto') 取界面 locale（默认 zh_CN）→ Global Rules 变中文
 *    （「不要硬编码」）；默认 en 含 Do NOT hardcode
...
test('③ 语言快切：en 含 Do NOT hardcode → 切 zh_CN 含「不要硬编码」', async () => {
  ...
  // 默认 en（settings.exportLang 默认 'en'）
  await expect.poll(() => readOutputBody(page)).toContain('Do NOT hardcode');

  // F11：语言快切 = 紧凑 2 项下拉（英文 / 跟随界面）。
  // 选「跟随界面」→ resolveLang('auto') 取界面 locale（默认 zh_CN）→ 中文输出。
  await clickShadowEl(page, 'pd-output-lang');
  await waitShadowVisible(page, '[data-testid="pd-dropdown"]');
  await clickShadowSel(page, '[data-testid="pd-dropdown"] [data-value="auto"]');
...
test('⑤ 关闭按钮关闭复制文本弹窗（F9 顶栏 X）', async () => {
  const page = await openFixturePage();
  await expandToolbar(page);
  ...
  // 点顶栏 X → 弹窗关闭
  await clickShadowEl(page, 'pd-output-close');
  await expect
    .poll(() =>
      page.evaluate(() => {
        const host = document.getElementById('pd-host');
